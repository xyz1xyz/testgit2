...
public class OutFormAction extends BaseAction{
	...
	public String formUI() throws Exception
	{
		...
		queryHelper.addCondition("w.type=?", "1");
		try {
			if(form!=null)
			{
				if(StringUtils.isNotBlank(form.getOutStorage()))
				{
					queryHelper.addCondition("w.outStorage like ?", "%" + form.getOutStorage() + "%");
